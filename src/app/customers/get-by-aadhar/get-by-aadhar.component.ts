...
export class GetByAadharComponent {
  ...
  constructor(private customerService:CustomersService,private toast:ToastrService){}
  ...
  getCustomerListDataByAadhar():void{
    ...
    this.customerService.getCustomerByAadharNo(this.searchByAadharForm.value.aadharNo).subscribe(
      (response)=>{
  ...
  getCustomerListDataByName():void{
    this.customerService.getCustomersBySimilarName(this.searchByNameForm.value.name).subscribe(
      (response)=>{
        if(response.length==0){
          this.toast.error("No customers found with matching name","Not found",{positionClass:'toast-bottom-right'});
          this.customerList=[];
        }
        else{
          this.customerList=response;
