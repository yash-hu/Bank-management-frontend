...
export class AccountsService {
  ...
  changeStatus(status:boolean,accountNo:number):Observable<string>{
    return this.httpclient.put(`${this.BaseUrl}/Update/${accountNo}`,status,{responseType:'text'});
  }

  deleteAccount(accountNo:number):Observable<string>{
    return this.httpclient.delete(`${this.BaseUrl}/Delete/${accountNo}`,{responseType:'text'});
  }

  addAccount(newAccountData:IAddAccountModel):Observable<string>{
    return this.httpclient.post(`${this.BaseUrl}/Add`,newAccountData,{responseType:'text'});
  }
