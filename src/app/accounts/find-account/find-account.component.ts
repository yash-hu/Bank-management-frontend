...
export class FindAccountComponent implements OnInit{
  ...
  searchGroup:FormGroup=new FormGroup({
    accountNo:new FormControl('',[Validators.pattern('^[0-9]{12}'),Validators.required])
  })

  handleSearch(){
    this.accountsService.getAccountByAccountNo(this.searchGroup.value.accountNo).subscribe(
      (data)=>{
        this.accountsList=[data];
      },
      (error)=>{
        this.toast.error(error.error,error.statusTest,{positionClass:'toast-bottom-right'});
        this.accountsList=[];
      }
    )
  }
